...
void Return_Running_State(Param_t *param){
  ...
  uint8_t 
  len = 0x04,
  return_buff[] = {HEADER, HEADER, len, param->error_type.No_err, param->Select_state, param->Set_Acceleration, param->Set_Braking};
      ...
      if(data_len == len_validation){
        ...
        // WRITE INSTRUCTION HANDLER ---------------------------------------------------------------
          ...
          // DATA ITEM RUNNING STATE
            ...
            if(rx_buff[5] == param->sub_item_type.Sub_item1) param->Select_state = rx_buff[6];
